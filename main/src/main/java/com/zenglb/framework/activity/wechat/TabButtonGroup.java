...
public class TabButtonGroup extends LinearLayout {
	 ...
	 */
	private static final int TEXT_START_COLOR= Color.GRAY,
	...
	//相关的资源ID：
	private final int ID_LAYOUT= R.layout.custom_tab_button,ID_IMAGE_TOP= R.id.custom_radio_button_image_top,ID_IMAGE_BOTTOM=R.id.custom_radio_button_image_botom,ID_TEXT=R.id.custom_radio_button_text,ID_NEWS=R.id.custom_radio_button_news;
	//条目变更监听
	...
	public TabButtonGroup(Context c) {
		super(c);
		init();
	}
	...
	public void itemChangeChecked(int leftIndex,int rightIndex,float alpha){
		...
		RadioButton leftBtn=lists.get(leftIndex);
		RadioButton rightBtn=lists.get(rightIndex);
		leftBtn.top.setAlpha(alpha);
		leftBtn.bottom.setAlpha(1-alpha);
		rightBtn.top.setAlpha(1-alpha);
		rightBtn.bottom.setAlpha(alpha);
		int aColor=getNewColor(1-alpha);
		int bColor=getNewColor(alpha);
		leftBtn.text.setTextColor(aColor);
		rightBtn.text.setTextColor(bColor);
	}
	/**
	 * 选择制定索引的条目
	 * 
